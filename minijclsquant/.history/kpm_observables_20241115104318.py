...
def random_vector(shape_H):
    return np.exp(1j * (np.random.uniform(0, 2*np.pi, shape_H)))


def delta_KPM_calc_two_vec(Ef,H,M,bounds_array,random_vector_l,random_vector_r):
    Ef=(Ef-(bounds_array[1]+bounds_array[0])/2)/((bounds_array[1]-bounds_array[0])/2)
    moments_jackson_delta_vec=moments_delta(Ef,M)*JacksonKernel(M)
    delta_0=random_vector_r
...
def delta_KPM_two_vec(H,M,bounds_array,random_vector_l,random_vector_r,Ef_vec):
    ...
    for i in range(len(Ef_vec)):
        delta_mat[i,1]=delta_KPM_calc_two_vec(Ef_vec[i],H,M,bounds_array,random_vector_l,random_vector_r)
    return delta_mat



def delta_KPM_calc_two_vec_2(H,M,random_vector_l,random_vector_r):


    delta_vec=np.zeros((M),dtype=complex)
    delta_0=random_vector_r
    delta_1=H.dot(random_vector_r)
    delta_vec[0]=np.vdot(random_vector_l,delta_0)
    delta_vec[1]=np.vdot(random_vector_l,delta_1)

    for m2 in range(2,M):
        delta_1,delta_0=2*H.dot(delta_1)-delta_0,delta_1
        delta_vec[m2]=np.vdot(random_vector_l,delta_1)
    return delta_vec


def delta_KPM_two_vec_2(H,M,bounds_array,random_vector_l,random_vector_r,Ef_vec):
    """
    Def:
        Mean value of the DOS with two different vectors making the table and the sum the moments
    Inputs:
        H: Hamiltonian in CSR or Ell.
        M: Number of moments.
        bounds_array: array of bounds such that bounds_array[0]=-bounds_array[1].
    ...
    delta_mat=np.zeros((len(Ef_vec),2))
    delta_mat[:,0]=Ef_vec
    Ef_vec=(Ef_vec-(bounds_array[1]+bounds_array[0])/2)/((bounds_array[1]-bounds_array[0])/2)
    delta_vec=delta_KPM_calc_two_vec_2(H,M,random_vector_l,random_vector_r)


    for i in range(len(Ef_vec)):
        delta_mat[i,1]=np.real(np.dot(moments_delta(Ef_vec[i],M)*JacksonKernel(M),delta_vec))
    return delta_mat
