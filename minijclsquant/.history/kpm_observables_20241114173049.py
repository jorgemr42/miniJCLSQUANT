...
def delta_KPM_calc_two_vec_2(Ef,H,M,bounds_array,random_vector_l,random_vector_r):
    Ef=(Ef-(bounds_array[1]+bounds_array[0])/2)/((bounds_array[1]-bounds_array[0])/2)
    moments_jackson_delta_vec=moments_delta(Ef,M)*JacksonKernel(M)
    delta_0=random_vector_r
    delta_1=H.dot(random_vector_r)
    ...
    delta_vec[:,0]=moments_jackson_delta_vec[0]*delta_0
    delta_vec[:,1]=moments_jackson_delta_vec[1]*delta_1

    for m2 in range(2,M):
        delta_1,delta_0=2*H.dot(delta_1)-delta_0,delta_1
        delta_vec[:,m2]=moments_jackson_delta_vec[m2]*delta_1
