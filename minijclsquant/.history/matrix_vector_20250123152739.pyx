...
cdef matrix_vector_2(int len_data, double complex[:] data,double complex[:] diagonal,long[:] indices, int len_row,double complex[:] v,double complex[:] v2,int n_threads):
    cdef Py_ssize_t i, j,idx
    cdef int num_columns = int(len_data/len_row)  
    if diagonal is None:
        for i in prange(num_columns,nogil='True',schedule='static',num_threads=n_threads):
