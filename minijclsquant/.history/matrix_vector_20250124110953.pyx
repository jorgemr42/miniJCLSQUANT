...
@cython.boundscheck(False)
@cython.cdivision(True)
def matrix_vector_2(Py_ssize_t num_columns, double complex[::1] data,double complex[:] diagonal,long[:] indices, int len_row,double complex[:] v,double complex[:] v2,int n_threads):
    cdef Py_ssize_t i, j, idx
    if diagonal is None:
        for i in prange(num_columns,nogil=True,schedule='static',num_threads=n_threads):
            for j in range(len_row):
