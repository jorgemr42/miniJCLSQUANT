...
def delta_KPM_calc_two_vec_2(H,M,random_vector_r):


    delta_vec=np.zeros((len(random_vector),M))
    delta_vec[:,0]=random_vector_r
    delta_vec[:,1]=H.dot(random_vector_r)

    for m2 in range(2,M):
        delta_vec[:,m2]=2*H.dot(delta_vec[:,m2-1])-delta_vec[:,m2-2]
    return delta_vec
...
def delta_KPM_two_vec_2(H,M,bounds_array,random_vector_l,random_vector_r,Ef_vec):
    ...
    Outputs:
        delta_mat: Array of shape (len(Ef_vec),2) where [:,0] are the unormalized Fermi energies and [:,1] the DOS.
    """moments_jackson_delta_vec[m2]*
    delta_mat=np.zeros((len(Ef_vec),2))
    delta_mat[:,0]=Ef_vec
    Ef_vec=(Ef_vec-(bounds_array[1]+bounds_array[0])/2)/((bounds_array[1]-bounds_array[0])/2)
    delta_vec=delta_KPM_calc_two_vec_2(H,M,random_vector_r)
    for i in range(len(Ef_vec)):
        moments_jackson_delta_vec=moments_delta(Ef_vec[i],M)*JacksonKernel(M)
        delta=np.real(np.vdot(random_vector_l,np.sum(moments_jackson_delta_vec)[:, np.newaxis],axis=0)))
