...
from .matrix_vector_module import matrix_vector,matrix_vector_2
import os
...
def data_indices(len_row,matrix_csr):
    indices=np.zeros(matrix_csr.shape[0]*len_row,dtype=np.int64)
    data=np.zeros(matrix_csr.shape[0]*len_row,dtype=np.complex128)
    for i in range(matrix_csr.shape[0]):
        row_i_data=matrix_csr[i,:].data
        row_i_indices=matrix_csr[i,:].indices
        if len(row_i_data)<len_row:
            indices[i*len_row:i*len_row+len(row_i_data)]=row_i_indices
            data[i*len_row:i*len_row+len(row_i_data)]=row_i_data

            indices[i*len_row+len(row_i_data):(i+1)*len_row]=row_i_indices[-1]*np.ones(len_row-len(row_i_data),dtype=np.int64)
            data[i*len_row+len(row_i_data):(i+1)*len_row]=np.zeros(len_row-len(row_i_data),dtype=np.complex128)

        else:
            indices[i*len_row:(i+1)*len_row]=row_i_indices
            data[i*len_row:(i+1)*len_row]=row_i_data
        return indices,data
...
class matrix_ell_c:
    ...
    def data_indices(len_row,A):
        indices=np.zeros(A.shape[0]*len_row,dtype=np.int64)
        data=np.zeros(A.shape[0]*len_row,dtype=np.complex128)
    ...
    def csr_to_ell(self):
        if has_diagonal_elements(self.A)==False:
            self.diagonal=None
        else:
            self.diagonal=self.A.diagonal()
            self.A=self.A-sci.sparse.diags(self.A.diagonal(),offsets=0,format='csr')

        # Determine the maximum number of non-zero elements in any row
        self.len_row = max(self.A.indptr[i+1] - self.A.indptr[i] 
                                for i in range(self.A.shape[0]))
        self.shape=self.A.shape
        self.indices,self.data=data_indices(self.len_row,self.A)
        return
    ...
    def dot(self,v):
        ...
        # return matrix_vector(self.data,self.diagonal, self.indices, self.len_row, v,v2, self.n_threads)
        return matrix_vector_2(self.shape[0],self.data,self.diagonal, self.indices, self.len_row, v,v2, self.n_threads)
