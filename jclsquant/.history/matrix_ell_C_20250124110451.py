...
class matrix_ell_c:
    ...
    def data_indices(self,A):
        indices=np.zeros(self.shape[0]*self.len_row,dtype=np.int64)
        data=np.zeros(self.shape[0]*self.len_row,dtype=np.complex128)
        for i in range(self.shape[0]):
            row_i_data=A[i,:].data
            row_i_indices=A[i,:].indices
            if len(row_i_data)<self.len_row:
                indices[i*self.len_row:i*self.len_row+len(row_i_data)]=row_i_indices
                data[i*self.len_row:i*self.len_row+len(row_i_data)]=row_i_data

                indices[i*self.len_row+len(row_i_data):(i+1)*self.len_row]=row_i_indices[-1]*np.ones(self.len_row-len(row_i_data),dtype=np.int64)
                data[i*self.len_row+len(row_i_data):(i+1)*self.len_row]=np.zeros(self.len_row-len(row_i_data),dtype=np.complex128)

            else:
                indices[i*self.len_row:(i+1)*self.len_row]=row_i_indices
                data[i*self.len_row:(i+1)*self.len_row]=row_i_data
    ...
    def dot(self,v):
        ...
        return matrix_vector_2(self.data.shape[0],self.data,self.diagonal, self.indices, self.len_row, v,v2, self.n_threads)
