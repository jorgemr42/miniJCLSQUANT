...
def delta_KPM_two_vec(H,M,bounds_array,random_vector_l,random_vector_r,Ef_vec):
    ...
    Outputs:
        delta_vec: Array of shape (len(Ef_vec),2) where [:,0] are the unormalized Fermi energies and [:,1] the DOS.
    """
    delta_vec=np.zeros(len(Ef_vec))
    for i in range(len(Ef_vec)):
        delta_vec[i]=delta_KPM_calc_two_vec_v4(Ef_vec[i],H,M,bounds_array,random_vector_l,random_vector_r)
