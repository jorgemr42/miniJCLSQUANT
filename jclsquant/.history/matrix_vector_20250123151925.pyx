...
def matrix_vector_2(Py_ssize_t len_data, double complex[:] data,double complex[:] diagonal,long[:] indices, Py_ssize_t len_row,double complex[:] v,double complex[:] v2,int n_threads):
    cdef Py_ssize_t i, j,idx
    cdef Py_ssize_t num_columns=len_data/len_row  
    if diagonal is None:
        for i in prange(num_columns,nogil='True',schedule='static',num_threads=n_threads):
