...
class matrix_ell_c:
    ...
    def check_v2(self):
        if self.v2 is None:
            self.v2=np.zeros(self.shape[0],dtype=np.complex128,order='C')
        return
    ...
    def dot(self,v):
        return matrix_vector(self.data,self.diagonal, self.indices, self.len_row, v, self.v2, self.n_threads)
