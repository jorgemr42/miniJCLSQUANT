...
def matrix_vector(data,diagonal, indices, len_row, v,n_threads):
    v_combined=np.zeros(2*len(v),dtype=np.complex128,order='C')
    v_copy=v_combined[:len(v)]
    v2=v_combined[len(v):]
    v_copy=np.copy(v)
    if diagonal is None:
        matrix_vector_c(data.shape[0],data, indices, len_row, v_copy,v2,n_threads)
    else:
        matrix_vector_c_diagonal(data.shape[0],data, diagonal,indices, len_row, v_copy,v2,n_threads)
    return v2 
