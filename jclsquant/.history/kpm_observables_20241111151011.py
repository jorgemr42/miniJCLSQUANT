...
# def delta_KPM_two_vec_v4(Ef_vec,H,M,bounds_array,random_vector_l,random_vector_r,number_process):
#     H=H/((bounds_array[1]-bounds_array[0])/2)-sci.sparse.identity(H.shape[0])*(((bounds_array[1]+bounds_array[0])/2)/((bounds_array[1]-bounds_array[0])/2))
#     delta_vec=np.zeros(len(Ef_vec))
#     j=0
#     with concurrent.futures.ThreadPoolExecutor(max_workers=number_process) as executor:
#         results=executor.map(delta_KPM_calc_two_vec_v4,Ef_vec,repeat(H),repeat(M),repeat(bounds_array),repeat(random_vector_l),repeat(random_vector_r)) #This will map the function with every item of the list This wont return the results as completed 
