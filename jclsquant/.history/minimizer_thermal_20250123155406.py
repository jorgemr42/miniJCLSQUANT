...
class problem:
    ...
    def emin_fun(self,x):
        # return abs(self.e_fun(x) - self.etarget)
        return (self.e_fun(x) - self.etarget)**2
    ...
    def demindu_fun(self,x):
        ...
        dedu = trapz( ((dfe_du+dfh_du)*self.E*self.dos)[self.icnp:-1] , dx=self.dE)
        # return np.sign(self.e_fun(x) - self.etarget) * dedu
        return 2*(self.e_fun(x) - self.etarget) * dedu
    ...
    def demindkT_fun(self,x):
        ...
        dedT = trapz( ((dfe_dkT+dfh_dkT)*self.E*self.dos)[self.icnp:-1] , dx=self.dE)
        # return np.sign(self.e_fun(x) - self.etarget) * dedT
        return 2*(self.e_fun(x) - self.etarget) * dedT
...
def minimization(deltaE,dos,initial_guess,niter=1000):
    ...
    for i in range(niter):
        ...
        # First make steepest descent toward target energy
        dedu  = p.dedu_fun([uold,k*Told])
        dedkT = p.dedkT_fun([uold,k*Told])
        #unew = uold + (p.etarget-ecurr) / dedu      * (dedu**2  / (dedu**2+dedkT**2))
        #Tnew = Told + (p.etarget-ecurr) / (dedkT*k) * (dedkT**2 / (dedu**2+dedkT**2))
        unew = uold + (p.etarget-ecurr)             * (dedu     / (dedu**2+dedkT**2))
        Tnew = Told + (p.etarget-ecurr) /        k  * (dedkT    / (dedu**2+dedkT**2))

        # Get new carrier density and updated mu,T
        ncurr = p.n_fun([unew,k*Tnew])
        uold  = unew
        Told  = Tnew
        uiter[2*i+1] = unew
        Titer[2*i+1] = Tnew

        # Then make steepest descent back to original carrier density
        dndu  = p.dndu_fun([uold,k*Told])
        dndkT = p.dndkT_fun([uold,k*Told])
        #unew = uold + (p.n0-ncurr) / dndu      * (dndu**2  / (dndu**2+dndkT**2))
        #Tnew = Told + (p.n0-ncurr) / (dndkT*k) * (dndkT**2 / (dndu**2+dndkT**2))
        unew = uold + (p.n0-ncurr)             * (dndu     / (dndu**2+dndkT**2))
        Tnew = Told + (p.n0-ncurr) /        k  * (dndkT    / (dndu**2+dndkT**2))

        # Get new energy and updated mu,T
        ecurr = p.e_fun([unew,k*Tnew])
        uold  = unew
        Told  = Tnew
        uiter[2*i+2] = unew
        Titer[2*i+2] = Tnew
    ...
    Tguess = Told
    # print()
    # print('uguess = '+str(uguess))
    # print('Tguess = '+str(Tguess))
    # print()
    ...
    unew = result.x[0]
    Tnew = result.x[1]/k
    nnew = p.n_fun([unew,k*Tnew])
    enew = p.e_fun([unew,k*Tnew])
    # print()
    # print('Minimizer success flag =',result.success)
    # print('Minimizer message =',result.message)
    # print()
    # print('u0   = '+str(u0))
    # print('T0   = '+str(T0))
    # print('unew = '+str(unew))
    # print('Tnew = '+str(Tnew))
    # print()
    # print('n0 - nnew = '+str(p.n0-nnew)+' e- / C-atom')
    # print('deltaE    = '+str(deltaE)+' eV / C-atom')
    # print('e0 - enew = '+str(enew-p.e0)+' eV / C-atom')
    # print('emin_fun  = '+str(p.emin_fun([unew,k*Tnew]))+' eV / C-atom')
    # print()
    return unew,Tnew*kb


def FD_optical_v2(E_vec,mu,T_mod,omega,W):
    if mu>-((omega/2)+1.76*0.5*W) and mu<((omega/2)+1.76*0.5*W) :
        return (1/(1+np.exp((E_vec-mu)/T_mod)))+(1/np.cosh((E_vec-omega/2)/W))-(1/np.cosh((E_vec+omega/2)/W))    
    else:
