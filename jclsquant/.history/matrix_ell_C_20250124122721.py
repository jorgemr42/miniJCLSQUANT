...
def data_indices(len_row,A_shape,A_indptr,A_data,A_indices):
    indices=np.zeros(A_shape*len_row,dtype=np.int64)
    data=np.zeros(A_shape*len_row,dtype=np.complex128)

    for i in numba.prange(A_shape):
        row_i_data=A_data[A_indptr[i]:A_indptr[i+1]]
        row_i_indices=A_indices[A_indptr[i]:A_indptr[i+1]]
...
class matrix_ell_c:
    ...
    def csr_to_ell(self):
        ...
        self.data,self.indices=data_indices(self.len_row,self.A.shape[0],self.A.indptr,self.A.data,self.A.indices)
        return
