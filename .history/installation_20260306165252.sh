...
read -p "Do you have installed pybinding? [yes/no] " pb_choice

if [[ "$pb_choice" == "no" ]]; then
    ...
    FILE="__init__.py"
    PATTERN="from jclsquant.pb_functions import *"
    sed -i "/$PATTERN/d" "$FILE"

    PATTERN="from jclsquant.jcl_paoflow import *"
    sed -i "/$PATTERN/d" "$FILE"

    PATTERN="from jclsquant.pybuild import *"
    sed -i "/$PATTERN/d" "$FILE"
