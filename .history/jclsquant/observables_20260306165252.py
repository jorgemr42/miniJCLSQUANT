...
def random_vector_generator_lattice(shape_H,N_lattices):
    """
    Def:
        Create a list of random vectors that are polarized in each (degree of freedom)
    Inputs:
        shape_H: Full shape of the Hamiltonian 
        N_lattices: Number of degrees of freedom in each unit cell.
                    (For example in graphene we have 2 atoms in the unit cell)
    Outputs:
        random_vector_list: List of random vectors each of them of shape_H with the form
                            [r1,0,r2,0,...] for example in the case of graphene. This list have N_lattices random vectos.
    """
    if shape_H % N_lattices != 0:
        raise ValueError("shape_H must be multiple of N_lattcies.")
    

    random_vector_list=[]
    for i in range(N_lattices):
        complex_numbers = np.exp(1j * 2 * np.pi * np.random.rand(shape_H // N_lattices))  # Generate r1, r2, ...
        result = np.zeros(shape_H, dtype=complex)
        result[i::N_lattices]=complex_numbers

        random_vector_list.append(result)
    return random_vector_list
...
def kpm_qgt(H, direction_1 = None , direction_2 = None ,Ef=None,M = None,random_vector=None):
    """
    Def:
        Quantum Geometric tensor, its real part gives the quantum metric and the imaginary part gives the Chern nuimber
    Inputs:
    ...
    ## Moments definition
    moments_kernel=moments_FD_T0(Ef/(0.5*(H.bounds[1]-H.bounds[0])),M)*JacksonKernel(M)
    
    ## Hamiltonian kpm reescalation
    H_kpm=H.modifier(modifier_bounds)


    if direction_1 == direction_2 or direction_2 is None:
        V1_kpm = H.modifier(modifier_velocity_bounds,direction_1)
        comm_1=rec_com_A_vec(M,H_kpm,V1_kpm,moments_kernel,random_vector)
        return vdot(1+0j,comm_1,rec_A_vec(M,H_kpm,moments_kernel,comm_1),H.shape[0],H.n_threads)
    elif direction_1 is None and direction_2 is None :
        print('No direction has been indicated')
        return 
    else:
        V1_kpm = H.modifier(modifier_velocity_bounds,direction_1)
        V2_kpm = H.modifier(modifier_velocity_bounds,direction_2)
        comm_1=rec_com_A_vec(M,H_kpm,V1_kpm,moments_kernel,random_vector)
        comm_2=rec_com_A_vec(M,H_kpm,V2_kpm,moments_kernel,random_vector)
        return vdot(1+0j,comm_2,rec_A_vec(M,H_kpm,moments_kernel,comm_1),H.shape[0],H.n_threads)

### Quantum geometric tensor
def kpm_qgt_m(H, direction_1 = None , direction_2 = None ,Ef=None,M = None,random_vector=None):
    """
    Def:
        Quantum Geometric tensor, its real part gives the quantum metric and the imaginary part gives the Chern nuimber
    Inputs:
    ...
    if direction_1 == direction_2 or direction_2 is None:
        V1_kpm = H.modifier(modifier_velocity_bounds_m,direction_1)
        comm_1=rec_com_A_vec(M,H_kpm,V1_kpm,moments_kernel,random_vector)
        return vdot(1+0j,comm_1,rec_A_vec(M,H_kpm,moments_kernel,comm_1),H.shape[0],H.n_threads)
    elif direction_1 is None and direction_2 is None :
        print('No direction has been indicated')
        ...
        V1_kpm = H.modifier(modifier_velocity_bounds_m,direction_1)
        V2_kpm = H.modifier(modifier_velocity_bounds_m,direction_2)
        comm_1=rec_com_A_vec(M,H_kpm,V1_kpm,moments_kernel,random_vector)
        comm_2=rec_com_A_vec(M,H_kpm,V2_kpm,moments_kernel,random_vector)
        return vdot(1+0j,comm_2,rec_A_vec(M,H_kpm,moments_kernel,comm_1),H.shape[0],H.n_threads)
...
def kpm_sigma_time(H,direction_1=None,direction_2=None,t_vec=None,Ef=None,Temp=None,M=None,random_vector=None,velocities=None):
    """
    Def:
    ...
    if t_vec is None:
        ...
        t_vec=np.linspace(0,5*hbar_fs*M/(pi*(0.5*(H.bounds[1]-H.bounds[0]))),500)
    if Temp is None:
        print('No temperature inserted by default is set to zero.')
        moments_kernel_FD=moments_FD_T0(Ef,M)*JacksonKernel(M)
    ...
    if Temp==0:
        moments_kernel_FD=moments_FD_T0(Ef/H.bounds[1],M)*JacksonKernel(M)
    else:
        moments_kernel_FD=moments_FD_T(Ef/H.bounds[1],Temp*kb/H.bounds[1],M)*JacksonKernel(M)
    ...
    aux=np.zeros(len(random_vector),dtype=np.complex128,order='C')
        
    for j in (range(len(t_vec))):
        if j==0:
            U=random_vector            
            U_commutator=rec_com_A_vec(M,H_kpm,V2_kpm,moments_kernel_FD,random_vector)
        else:
            U=rec_A_vec(M2,H_kpm,moments_U_vec,U)
            U_commutator=rec_A_vec(M2,H_kpm,moments_U_vec,U_commutator)

        V1.dot(1.0+0j,0+0j,U,aux)
        sigma_vec[j]=vdot(0-1j,U_commutator,aux,H.shape[0],H.n_threads)
...
def kpm_msd(H,direction_2=None,t_vec=None,M=None,random_vector=None,velocities=None):
    """
    Def:
        Time-dependent conducitivity
    Inputs:
    ...
    for j in (range(len(t_vec))):
        if j==0:
            mean_x=random_vector
            U=random_vector
        elif j==1:
            mean_x=rec_com_A_vec(M2,H_kpm,V2_kpm,moments_U_vec,random_vector)

            msd_vec[:,j]=rec_A_tab(M,H_kpm,mean_x)
            
            U=rec_A_vec(M2,H_kpm,moments_U_vec,random_vector)
        else:
            mean_x=rec_A_vec(M2,H_kpm,moments_U_vec,mean_x)
            
            axpy2(1+0j,1+0j,rec_com_A_vec(M2,H_kpm,V2_kpm,moments_U_vec,U),mean_x, H_kpm.shape[0],H_kpm.n_threads)
    ...
    for i in range(len(E_vec)):
        ...
        for j in range(len(t_vec)):
            msd_mat[i,j]=np.real(dot(1+0j,moments_kernel,np.ascontiguousarray(msd_vec[:,j]),M,H.n_threads))/dos[i,1]
