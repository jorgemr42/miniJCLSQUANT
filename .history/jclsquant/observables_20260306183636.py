...
def kpm_n_dos_n(H,M=None,random_vector_l=None,random_vector_r=None,bounds=True,proyector=None):
    ...
    if proyector is not None:
        ...
        if bounds==True:
            ...
            H_kpm=H.modifier(modifier_bounds)
            dos_n_vec=rec_A_tab2v(M,H_kpm,random_vector_l_2,random_vector_r)

        elif bounds==False:

            dos_n_vec=rec_A_tab2v(M,H,random_vector_l_2,random_vector_r)
    else:
        if bounds==True:

            H_kpm=H.modifier(modifier_bounds)
            dos_n_vec=rec_A_tab2v_2(M,H_kpm,random_vector_l,random_vector_r)

        elif bounds==False:

            dos_n_vec=rec_A_tab2v_2(M,H,random_vector_l,random_vector_r)
