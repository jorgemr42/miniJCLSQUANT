...
def load_sparse_coo(filename):
    ...
    with open(filename, 'r') as f:
        ...
        header = f.readline()
        dim, nnz = map(int, header.strip().split())
    ...
    # Create COO matrix
    coo = sci.sparse.coo_matrix((data, (row, col)), shape=(dim, dim))
...
def H_haldane(positions, t,t2,m=0,W=0,periodic=True,type_H='CSR'):
    ...
    if np.abs(m)!= 0:
        ...
        conjugate_mat = conjugate_mat + sci.sparse.diags(np.array([2] * N), offsets=0, shape=(N, N), format="csr")
...
def H_AB_bilayer(positions, gamma0,gamma1=0.4,m=0,W=0,E=0,periodic=True,type_H='ELL'):
    ...
    if np.abs(m)!= 0:
        ...
        diag[0::2]=np.array([m, -m] * (N//4))
        diag[1::2]=np.array([m, -m] * (N//4))
        H+=sci.sparse.diags(diag, offsets=0, format="csr")
...
def H_AB_bilayer_2(positions, gamma0,gamma1=0.4,m=0,W=0,E=0,periodic=True,type_H='ELL'):
    ...
    if np.abs(m)!= 0:
        ...
        H+=sci.sparse.diags(np.concatenate((-m*np.ones(N//2),m*np.ones(N//2))), offsets=0, format="csr")
        conjugate_mat.setdiag(np.array([2, 2] * (N//2)))
    if np.abs(W)!= 0:
        H+=sci.sparse.diags((W)*(2*np.random.rand(H.shape[0])-1), offsets=0, format="csr")
    ...
    if np.abs(E)!= 0:
        ...
        diag = np.array([-E if i % 2 == 0 else -E for i in range(N)])
        # Or using numpy methods:
        # diag = np.ones(N) * m
        # diag[1::2] = -m
