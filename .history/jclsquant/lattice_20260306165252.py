...
def lattice_AB_bilayer(N):
    # You need N to be of the form 2**(4+something)
    a = 0.24595   # [nm] unit cell length
    a_cc = a/np.sqrt(3)  # [nm] carbon-carbon distance
    a2=np.array([0,a_cc])
    ...
    S=lattice_hexagonal(N//2)
    S_tot[:N//2,:2]=S
    S_tot[N//2:,0]=S[:,0]+a2[0]
    S_tot[N//2:,1]=S[:,1]+a2[1]
    S_tot[N//2:,2]+=d
    return S_tot

def lattice_AB_bilayer_2(N):
    # You need N to be of the form 2**(4+something)
    a = 0.24595   # [nm] unit cell length
    a_cc = a/np.sqrt(3)  # [nm] carbon-carbon distance
    a2 = np.array([0, a_cc])
    ...
    S_tot = np.zeros((2*np.shape(S)[0], 3))
    
    # Create intercalated structure
    S_tot[0::2, :2] = S  # Even indices: first layer
    S_tot[1::2, 0] = S[:, 0] + a2[0]  # Odd indices: second layer x-coordinates
    S_tot[1::2, 1] = S[:, 1] + a2[1]  # Odd indices: second layer y-coordinates
    S_tot[1::2, 2] = d  # Odd indices: second layer z-coordinates

    return S_tot

def lattice_haxagonal_quiral(N):
    if N%9!=0:
        print('The number of sites must be of the form N= 9*2*(3+x) where x is odd as Chiral phonons should be 3 periodic in the x direction.')
        return 
    else: 
        return lattice_hexagonal(N)
